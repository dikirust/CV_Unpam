...
# -*- coding: utf-8 -*-
"""
Generate JUTIF-compliant journal article with improved Indonesian content
All technical terms (framework, library, function) remain in English and italic
...
print("=" * 80)
print("GENERATING JUTIF-COMPLIANT JOURNAL ARTICLE (IMPROVED VERSION)")
print("=" * 80)
...
run = title.add_run("Perbandingan Arsitektur Pembelajaran Mendalam untuk Klasifikasi Sampah: Analisis Jaringan Saraf Konvolusional Khusus dan MobileNetV2")
run.font.size = Pt(13)
run.font.bold = True
...
abstract_runs = [
    ...
    ("CNN", True),
    (" khusus yang dibangun dari awal dengan MobileNetV2 yang menerapkan pembelajaran transfer dari bobot ImageNet pre-terlatih. Dataset terdiri dari 8.400 citra sampah dengan distribusi yang seimbang di antara lima kelas. Praproses meliputi pengubahan ukuran citra menjadi 64×64 piksel dan normalisasi nilai piksel ke rentang 0-1. Model ", False),
    ("CNN", True),
    (" khusus menggunakan tiga blok konvolusional dengan progres filter 32→64→128, normalisasi batch, dan lapisan dropout untuk regularisasi. MobileNetV2 memanfaatkan konvolusi separabel mendalam dan blok residu terbalik dengan model basis pre-terlatih dari ImageNet, kemudian ditambahkan kepala khusus dengan ", False),
    ...
    (", ukuran batch 16, dan penghentian awal dengan kesabaran 10. Hasil evaluasi menunjukkan MobileNetV2 mencapai akurasi 93.65% dengan presisi 93.68%, ingatan 93.72%, dan F1-Skor 93.70%, secara signifikan lebih baik dari ", False),
    ("CNN", True),
    (" khusus yang mencapai akurasi 90.16%. Kecepatan pelatihan MobileNetV2 juga dua kali lebih cepat (231 detik vs 451 detik). Analisis akurasi per-kelas menunjukkan kedua model memiliki performa tinggi di semua kelas dengan variasi minimal. Hasil penelitian membuktikan bahwa pembelajaran transfer memberikan keunggulan dalam hal akurasi, kecepatan konvergensi, dan efisiensi parameter dibandingkan dengan arsitektur khusus pada dataset terbatas.", False),
]
...
keywords_list = [
    ...
    ("Visi Komputer", False),
    (", ", False),
    ("Pembelajaran Mendalam", False),
    (", MobileNetV2, ", False),
    ("Pembelajaran Transfer", False),
]
...
p2_runs = [
    ("Teknologi ", False),
    ("visi komputer", True),
    (" berbasis pembelajaran mendalam telah menunjukkan potensi luar biasa dalam mengotomatisasi tugas-tugas visual yang kompleks, termasuk deteksi, segmentasi, dan klasifikasi objek [8], [9]. Jaringan Saraf Konvolusional (", False),
    ("CNN", True),
    (") merupakan arsitektur pembelajaran mendalam yang paling efektif untuk pemrosesan citra karena kemampuannya dalam mengekstraksi fitur hirarki melalui operasi konvolusi [10], [11]. Aplikasi ", False),
    ("CNN", True),
...
p3_runs = [
    ("Pembelajaran transfer menawarkan solusi alternatif dengan memanfaatkan pengetahuan yang telah dipelajari dari dataset besar seperti ", False),
    ("ImageNet", True),
    (" untuk meningkatkan performa model pada dataset kecil [14], [15]. MobileNetV2 merupakan arsitektur ringan yang dirancang khusus untuk penyebaran di perangkat ", False),
    ("mobile", True),
    (" dengan sumber daya terbatas tanpa mengorbankan akurasi secara signifikan [16], [17]. Konvolusi separabel mendalam yang digunakan dalam MobileNetV2 mengurangi jumlah parameter model hingga 10 kali lipat dibandingkan konvolusi standar sambil mempertahankan atau bahkan meningkatkan akurasi [18], [19].", False),
]
...
intro_p4_text = "Meskipun berbagai penelitian telah membandingkan algoritma klasifikasi untuk sampah, masih terbatas jumlah studi yang melakukan perbandingan menyeluruh antara arsitektur khusus dengan pembelajaran transfer pada dataset sampah yang sama dengan fokus pada metrik evaluasi komprehensif. Perbedaan pertukaran antara akurasi, kecepatan pelatihan, efisiensi parameter, dan skalabilitas belum dikaji secara detail. Penelitian ini dirancang untuk mengisi celah pengetahuan tersebut dengan melakukan perbandingan sistematis antara CNN khusus dan MobileNetV2 pada dataset sampah berkualitas tinggi yang mencakup lima jenis sampah umum."
...
run = sub_a.add_run("A. Dataset dan Praproses")
run.font.size = Pt(11)
run.font.bold = True
run.font.name = 'Times New Roman'
...
d_runs = [
    ...
    ("plastic", True),
    (") 2.496 citra (29.7%). Distribusi yang tidak sepenuhnya seimbang mencerminkan komposisi sampah nyata di lapangan, dengan plastik dan kertas menjadi komponen dominan. Praproses data melibatkan beberapa tahapan: (1) Muat citra dalam format ", False),
    ("RGB", True),
...
run = sub_b.add_run("B. Arsitektur CNN Khusus")
run.font.size = Pt(11)
run.font.bold = True
run.font.name = 'Times New Roman'
...
c_runs = [
    ("CNN khusus dibangun dengan tiga blok konvolusional yang dirancang untuk menangkap fitur pada tingkat abstraksi berbeda [10], [11]. Setiap blok mengikuti pola: lapisan ", False),
    ("Conv2D", True),
    (" dengan ukuran kernel 3×3 dan aktivasi ", False),
    ("ReLU", True),
    (", diikuti normalisasi batch untuk stabilisasi pelatihan [22], ", False),
    ("Max Pooling", True),
    ...
    ("Conv2D", True),
    ("(32) mengekstraksi fitur tingkat rendah seperti tepi; Blok 2 menggunakan ", False),
    ("Conv2D", True),
    ("(64) mengekstraksi fitur tingkat menengah seperti bentuk lokal; Blok 3 menggunakan ", False),
    ("Conv2D", True),
    ("(128) mengekstraksi fitur tingkat tinggi seperti struktur global. Setelah blok konvolusional, dilakukan ", False),
    ("Global Average Pooling", True),
...
run = sub_c.add_run("C. Pembelajaran Transfer dengan MobileNetV2")
run.font.size = Pt(11)
run.font.bold = True
run.font.name = 'Times New Roman'
...
m_runs = [
    ("MobileNetV2 merupakan arsitektur yang dirancang untuk penyebaran di perangkat ", False),
    ("mobile", True),
    (" dengan lebar pita dan sumber daya komputasi terbatas [16], [17]. Arsitektur menggunakan konvolusi separabel mendalam yang memisahkan konvolusi standar menjadi konvolusi mendalam dan konvolusi titik [18], mengurangi biaya komputasi dan jumlah parameter hingga 10 kali dibandingkan konvolusi standar [19]. Blok fundamental MobileNetV2 adalah blok residu terbalik dengan desain leher botol [16]. Pembelajaran transfer menggunakan bobot pre-terlatih dari pelatihan dataset ", False),
    ("ImageNet", True),
    (" yang besar (lebih dari 1 juta citra pada 1.000 kategori) [24]. Strategi penyesuaian halus: (1) Muat model basis MobileNetV2 pre-terlatih dengan bobot ImageNet dan bekukan semua lapisan [14], [15]; (2) Kepala khusus terdiri dari ", False),
    ("Global Average Pooling", True),
    ("; (3) ", False),
    ("Dense", True),
    ("(256) dengan aktivasi ", False),
    ("ReLU", True),
    ("; (4) ", False),
    ("Dense", True),
    ("(128) dengan aktivasi ", False),
    ("ReLU", True),
    ("; (5) Keluaran ", False),
    ("Dense", True),
    ("(5) dengan aktivasi ", False),
    ("Softmax", True),
...
t_runs = [
    ...
    ("Adam", True),
    (" (Estimasi Momen Adaptif) dipilih dengan laju pembelajaran 0.001 [25]. Fungsi kerugian ", False),
    ("Sparse Categorical Crossentropy", True),
    (" digunakan untuk klasifikasi multi-kelas dengan label kelas integer. Ukuran batch 16 dipilih sebagai kompromi antara efisiensi memori dan stabilitas gradien. Epoch maksimal diset ke 50, namun pelatihan dihentikan lebih awal jika kerugian validasi tidak meningkat selama 10 epoch berturut-turut melalui panggilan balik penghentian awal [26]. Penjadwal Laju Pembelajaran dengan panggilan balik ", False),
...
r_runs = [
    ...
    (", dan F1-Skor ", False),
    (f"{mobilenet_results['f1_score']:.2%}", True),
    ...
    (", dan F1-Skor ", False),
    (f"{custom_cnn_results['f1_score']:.2%}", True),
...
eff_runs = [
    ...
    (" lebih cepat. Perbedaan waktu pelatihan ini signifikan secara praktis, terutama ketika melakukan penyesuaian hiperparameter ekstensif atau pelatihan pada dataset lebih besar [16].", False),
]
...
image_files = [
    ...
    (REPORT_DIR / 'confusion_matrices.png', 'Gambar 2. Matriks Kebingungan kedua model'),
...
disc_runs = [
    ("Pembelajaran transfer dari bobot ImageNet pre-terlatih memberikan representasi awal yang sudah optimal untuk ekstraksi fitur umum seperti tepi, tekstur, dan bentuk [14], [15]. Dataset sampah 8.400 citra mungkin masih relatif terbatas untuk pelatihan CNN khusus dari awal yang memerlukan jumlah parameter besar. Arsitektur MobileNetV2 dengan konvolusi separabel mendalam secara inheren lebih efisien namun tetap mampu untuk mengekstraksi fitur kompleks [18]. Konvergensi lebih cepat pada MobileNetV2 menunjukkan bahwa lanskap masalah optimasi lebih menguntungkan, kemungkinan karena desain arsitektur yang telah dioptimalkan melalui penelitian ekstensif dan patokan [16], [19].", False),
]
...
concl_runs = [
    ("Penelitian ini telah berhasil mengembangkan, melatih, dan mengevaluasi dua arsitektur jaringan saraf konvolusional untuk tugas klasifikasi sampah lima kelas [12], [13]. Temuan-temuan utama adalah: (1) MobileNetV2 dengan pembelajaran transfer menunjukkan performa superior dibandingkan CNN khusus dengan akurasi ", False),
    (f"{mobilenet_results['accuracy']:.2%}", True),
    (" vs ", False),
    (f"{custom_cnn_results['accuracy']:.2%}", True),
    (" [27]; (2) Pembelajaran transfer dari bobot ImageNet pre-terlatih memberikan keunggulan yang menentukan untuk dataset ukuran sedang seperti 8.400 citra [14], [15]; (3) MobileNetV2 menunjukkan efisiensi komputasi yang superior dengan pelatihan ", False),
    (f"{(training_times['custom_cnn']/training_times['mobilenetv2']):.1f}x", True),
    (" lebih cepat [16], [17]; (4) Performa tinggi di semua lima kategori sampah menunjukkan bahwa kedua model telah belajar representasi visual yang kokoh; (5) Pembelajaran transfer harus menjadi pendekatan bawaan untuk tugas klasifikasi dengan dataset terbatas dalam aplikasi visi komputer. Rekomendasi praktis adalah menggunakan MobileNetV2 untuk implementasi klasifikasi sampah dunia nyata yang memerlukan keseimbangan antara akurasi tinggi dan efisiensi komputasi.", False),
]
...
print("\n" + "=" * 80)
print("JUTIF JOURNAL ARTICLE GENERATED SUCCESSFULLY (IMPROVED VERSION)!")
print("=" * 80)
print(f"\nOutput file: {docx_file}")
print(f"File size: {docx_file.stat().st_size / 1024:.1f} KB")
print("\nImprovements applied:")
print("  ✓ Istilah Bahasa Indonesia maksimal (visi komputer, pembelajaran transfer, dll)")
print("  ✓ Semua istilah Inggris dalam italic font")
print("  ✓ Sitasi sesuai dengan konten (25 referensi IEEE)")
print("  ✓ Total parameter terlatih (bukan trainable parameters)")
print("  ✓ Professional formatting: Times New Roman, margins proper")
print("\n" + "=" * 80)
sys.stdout.flush()
