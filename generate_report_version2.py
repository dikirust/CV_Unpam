...
# -*- coding: utf-8 -*-
"""
Generate JUTIF-compliant journal article based on provided text
All English technical terms are formatted in italic
"""
...
print("=" * 80)
print("GENERATING FINAL REPORT FROM PROVIDED TEXT")
print("=" * 80)
...
def add_paragraph_with_italics(doc, text, size=11):
    """Add paragraph with automatic italic formatting for English terms"""
    # Dictionary of English terms to italicize
    english_terms = [
        'computer vision', 'convolutional neural network', 'CNN', 'transfer learning',
        'ImageNet', 'pre-trained', 'weights', 'Dataset', 'Preprocessing', 'resize',
        'pixel', 'normalization', 'Custom CNN', 'convolutional', 'filter',
        'batch normalization', 'dropout', 'layers', 'regularisasi', 'MobileNetV2',
        'depthwise separable convolutions', 'inverted residual blocks', 'base model',
        'custom head', 'Global Average Pooling', 'dense', 'training', 'optimizer',
        'Adam', 'learning rate', 'loss function', 'Sparse Categorical Crossentropy',
        'batch size', 'early stopping', 'patience', 'accuracy', 'precision', 'recall',
        'F1-Score', 'per-class', 'konvergensi', 'custom architecture'
    ]
    ...
    for word in words:
        ...
        # Check if word contains English term
...
# Process abstract - simple approach: add with basic formatting first
for run in abstract_para.runs:
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'

# Simpler approach: just add the text with manual formatting where needed
abstract_segments = [
    ...
    (" untuk klasifikasi lima jenis sampah: sampah organik, kaca, logam, kertas, dan plastik. Perbandingan dilakukan antara Custom ", False),
    ("CNN", True),
    ...
    (" nilai ", False),
    ("pixel", True),
    (" ke rentang 0-1. Model Custom ", False),
    ("CNN", True),
    ...
    (" ", False),
    ("pre-trained", True),
    (" dari ImageNet, kemudian ditambahkan ", False),
    ("custom head", True),
    (" dengan ", False),
    ...
    (" 93.72%, dan ", False),
    ("F1-Score", True),
    (" 93.70%, secara signifikan lebih baik dari Custom ", False),
    ("CNN", True),
    (" yang mencapai ", False),
    ("accuracy", True),
...
# Intro paragraphs with mixed content (will add basic formatting)
intro_paragraphs = [
...
intro_para_mapping = [
    # Para 1 - general waste management
    [
    ...
    [
        ...
        ("CNN", True),
        (" dari awal memerlukan dataset berukuran besar dan komputasi intensif, sementara dataset sampah yang tersedia umumnya terbatas dalam jumlah dan variasi [13], [14].", False),
    ],
    # Para 3 - transfer learning
    [
        ("Transfer learning menawarkan solusi alternatif dengan memanfaatkan pengetahuan yang telah dipelajari dari dataset besar seperti ImageNet untuk meningkatkan performa model pada dataset kecil [16], [17]. MobileNetV2 merupakan arsitektur lightweight yang dirancang khusus untuk deployment di perangkat mobile dengan resource terbatas tanpa mengorbankan akurasi secara signifikan [18], [20]. ", False),
        ("Depthwise separable convolutions", True),
        (" yang digunakan dalam MobileNetV2 mengurangi jumlah parameter model hingga 10 kali lipat dibandingkan standard convolutions sambil mempertahankan atau bahkan meningkatkan akurasi [19], [20].", False),
    ],
    # Para 4
    [
        ("Meskipun berbagai penelitian telah membandingkan algoritma klasifikasi untuk sampah, masih terbatas jumlah studi yang melakukan perbandingan menyeluruh antara custom architecture dengan transfer learning pada dataset sampah yang sama dengan fokus pada metrik evaluasi komprehensif. Perbedaan trade-off antara akurasi, kecepatan training, efisiensi parameter, dan skalabilitas belum dikaji secara detail. Penelitian ini dirancang untuk mengisi celah pengetahuan tersebut dengan melakukan perbandingan sistematis antara Custom CNN dan MobileNetV2 pada dataset sampah berkualitas tinggi yang mencakup lima jenis sampah umum.", False),
    ...
    # Para 5
    [
        ("Tujuan penelitian ini adalah: (1) mengembangkan dan melatih Custom ", False),
        ("CNN", True),
        (" untuk klasifikasi lima jenis sampah; (2) mengimplementasikan MobileNetV2 dengan ", False),
        ("transfer learning", True),
        (" dari ImageNet weights; (3) melakukan evaluasi komprehensif menggunakan metrik accuracy, precision, recall, F1-Score, dan confusion matrix; (4) menganalisis performa per-class untuk setiap jenis sampah; (5) membandingkan trade-off antara akurasi dan efisiensi komputasi; (6) memberikan rekomendasi arsitektur yang paling sesuai untuk aplikasi praktis. Dengan menyelesaikan tujuan-tujuan ini, penelitian diharapkan dapat memberikan panduan praktis untuk pengembangan sistem klasifikasi sampah otomatis yang efisien dan akurat.", False),
    ],
]

for para_segments in intro_para_mapping:
    para = doc.add_paragraph()
...
dataset_para_content = [
    ...
    ("Preprocessing", True),
    (" data melibatkan beberapa tahapan: (1) Load citra dalam format ", False),
    ("RGB", True),
    ("; (2) ", False),
    ("Resize", True),
    (" semua citra menjadi ukuran standar 64x64 ", False),
    ("pixel", True),
    (" untuk input ke neural network [23]; (3) Normalisasi nilai ", False),
    ("pixel", True),
    (" dari rentang 0-255 menjadi float 0-1 [23]; (4) Stratified split data dengan proporsi 70% training, 15% validation, dan 15% testing untuk memastikan representative distribution setiap kelas di setiap subset.", False),
]
...
cnn_content = [
    ...
    ("Conv2D", True),
    (" layer dengan kernel size 3x3 dan ", False),
    ("ReLU", True),
    (" activation, diikuti ", False),
    ("Batch Normalization", True),
    (" untuk stabilisasi training [24], ", False),
    ("Max Pooling", True),
    (" dengan stride 2x2 untuk downsampling spatial dimensions, dan ", False),
    ("Dropout", True),
    ...
    ("Global Average Pooling", True),
    (" mengagregasi spatial information menjadi feature vector 128-dimensional. Dua ", False),
    ("Dense", True),
    (" layers dengan masing-masing 256 dan 128 units memberikan representasi non-linear dari features yang telah diekstraksi. Output layer dengan 5 units dan ", False),
    ("Softmax", True),
    (" activation menghasilkan probabilitas prediksi untuk setiap kelas sampah. Total trainable parameters adalah 310.405.", False),
]
...
mobile_content = [
    ("MobileNetV2 merupakan arsitektur yang dirancang untuk deployment di perangkat mobile dengan bandwidth dan computational resources terbatas [18], [20]. Arsitektur menggunakan ", False),
    ("depthwise separable convolutions", True),
    (" yang memisahkan standard convolution menjadi depthwise convolution dan pointwise convolution [19], mengurangi computational cost dan jumlah parameters hingga 10 kali dibandingkan standard convolutions [20]. Blok fundamental MobileNetV2 adalah ", False),
    ("inverted residual block", True),
    (" dengan bottleneck design [18]. Transfer learning menggunakan pre-trained weights dari training ImageNet dataset yang massive (lebih dari 1 juta citra pada 1.000 kategori) [21]. Strategi ", False),
    ("fine-tuning", True),
    (": (1) Load pre-trained MobileNetV2 base model dengan ImageNet weights dan freeze semua layers [16], [17]; (2) Custom head terdiri dari ", False),
    ("Global Average Pooling", True),
    ("; (3) ", False),
    ("Dense(256)", True),
    (" dengan ", False),
    ...
    (" dengan ", False),
    ("ReLU", True),
    (" activation; (5) Output ", False),
    ("Dense(5)", True),
    (" dengan ", False),
    ("Softmax", True),
    (" activation. Freezing base layers mencegah perubahan bobot pre-trained yang already optimal, sementara training custom head memungkinkan adaptasi terhadap karakteristik dataset sampah [16].", False),
]
...
train_content = [
    ("Kedua model dilatih menggunakan konfigurasi uniform untuk memastikan fair comparison. Optimizer ", False),
    ("Adam", True),
    (" (Adaptive Moment Estimation) dipilih dengan learning rate 0.001 [26]. Loss function ", False),
    ("Sparse Categorical Crossentropy", True),
    (" digunakan untuk multi-class classification dengan integer class labels. Batch size 16 dipilih sebagai compromise antara memory efficiency dan gradient stability. Maximum epochs diset ke 50, namun training dihentikan lebih awal jika validation loss tidak meningkat selama 10 epochs berturut-turut melalui ", False),
    ("Early Stopping", True),
    (" callback [27]. Learning Rate Scheduler dengan ", False),
    ("ReduceLROnPlateau", True),
    (" callback mengurangi learning rate dengan faktor 0.5 jika validation loss plateau selama 5 epochs.", False),
]
...
result_content = [
    ("MobileNetV2 menunjukkan keunggulan signifikan dalam semua metrik evaluasi dibandingkan Custom ", False),
    ("CNN", True),
...
perclass_content = [
    ("Analisis akurasi per-kelas menunjukkan bahwa kedua model mencapai performa tinggi di semua kategori sampah. Custom ", False),
    ("CNN", True),
    (" mencapai ", False),
    ("accuracy", True),
...
efficiency_content = [
    ("Aspek penting lainnya adalah analisis efficiency dan scalability. Custom ", False),
    ("CNN", True),
    (" memerlukan waktu training 4440 detik (~74.0 menit). MobileNetV2 berhasil dalam waktu 754 detik (~12.6 menit), menghasilkan speedup sebesar 5.9x lebih cepat. Perbedaan waktu training ini signifikan secara praktis, terutama ketika melakukan extensive hyperparameter tuning atau training pada dataset lebih besar [18].", False),
]
...
# Discussion
disc_content = [
    ("Transfer learning dari pre-trained ImageNet weights memberikan initial representations yang sudah optimal untuk feature extraction umum seperti edges, textures, dan shapes [16], [17]. Dataset sampah 8.400 citra mungkin masih relatif terbatas untuk training Custom ", False),
    ("CNN", True),
    (" dari awal yang memerlukan jumlah parameters besar. MobileNetV2 architecture dengan ", False),
    ("depthwise separable convolutions", True),
    (" inherently lebih efficient namun tetap capable untuk mengekstraksi features kompleks [19]. Convergence lebih cepat pada MobileNetV2 menunjukkan bahwa landscape optimization problem lebih favorable, kemungkinan karena architecture design yang telah dioptimalkan melalui extensive research dan benchmarking [18], [20].", False),
]
...
print("\nDocument features:")
print("  ✓ Content from provided text (exact structure)")
print("  ✓ All English technical terms in italic")
print("  ✓ 27 IEEE-format references")
print("  ✓ Times New Roman 11pt, proper formatting")
print("  ✓ 3 PNG visualizations embedded")
print("  ✓ Professional academic layout")
print("\n" + "=" * 80)
sys.stdout.flush()
